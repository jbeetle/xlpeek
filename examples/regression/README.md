...
| `regress_round2.py` | `testdata/round2.xlsx` 五张表，第二轮外部复检的 15 条各一个最小用例（多数断言的是**报错**） | 上游的验收方式是「逐条独立复现」，所以每条缺陷都要有一个能独立失败的命令；3 条静默错值的用例断言的是"必须报错/必须告警"，而不是"结果正确" |
...
`_common.py` 在导入时做一次预检：二进制能不能跑（`version` 有没有信封）、
`REQUIRED_FIXTURES` 里每个夹具在不在。任一项不满足就打印缺失清单并以 1 退出，
**不进入任何断言**。

这一条是被上游逼出来的：交付给对方的包漏了 `testdata/` 时，九套回归的每一条断言
都报 `FILE_NOT_FOUND`，其中一套还在打印结果前直接抛 `KeyError: 'data'` 崩掉——
看起来像"工具坏了"，实际是"包缺文件"。现在缺什么就说什么。
...
它由 openpyxl 生成（公式列必须**不带缓存值**，所以不能用 Excel 存）；`regress_shapes.py`
的期望值是**手写死**的，改夹具必须同步改期望。

`testdata/round2.xlsx` 是**可重新生成**的，生成脚本随仓库提交：

```bash
python examples/regression/make_fixtures.py     # 需要 openpyxl，仅此一步需要
```

五张表各一种形状（`筛选` = 数值列 + 百分比列；`日期` = 同一天写成三种数字格式，
外加一列**文本**日期；`尾部空行` = 两行数据 + 195 行只有格式的空行；`括号列名` = `金额(万元)`；
`高基数` = 1200 行近似唯一的凭证号）。提交生成脚本是为了让夹具的形状可被追问：
"这个文件到底想表达什么"不该只有一个人的记忆能回答。跑完再跑一遍套件，
就证明它还表达着那些期望说的东西。
